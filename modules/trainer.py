...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        # 训练循环：直接使用collate_fn处理好的批次张量
        for batch_idx, (images_id, images, reports_ids, reports_masks, _) in enumerate(self.train_dataloader):
            # 关键修正：images已是(batch_size, max_num_images, C, H, W)张量，直接移至设备
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks, _) in enumerate(self.val_dataloader):
                images = images.to(self.device)  # 直接移至设备，无需stack
                reports_ids, reports_masks = reports_ids.to(self.device), reports_masks.to(self.device)
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            test_report_path = os.path.join(self.checkpoint_dir, "test_LGK.txt")
